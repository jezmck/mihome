		...
		request({
			...
			try {
				...
				if (json.status === 'success') {
					resolve(json['data'].api_key);
				} else {
					reject(`Request Error: ${json.status}`);
				}
			} catch (err) {
				reject(err);
			}
...
function simpleGET(endpoint, focus) {
	return new Promise((resolve, reject) => {
 ...
 */
function postWithID(endpoint, id, focus) {
	return new Promise((resolve, reject) => {
		const URL = `${BASE_URL}${endpoint}`;
		request({
			url: URL,
			...
			if (body.status !== 'success') {
				reject(`Request Error: ${body.status}`);
			} else {
				let response = body;
				if (focus) {
...
function fetchUsageData(id, data_type, resolution, start_time, end_time, limit) {
	return new Promise((resolve, reject) => {
		const dataType = data_type.toLocaleString().toLowerCase();
		const resolutionLC = resolution.toLocaleString().toLowerCase();
		if (dataType !== 'watts' || dataType !== 'reported_temperature') {
			reject(`Data Type must be 'watts' or 'reported_temperature', you provided. ${data_type}`);
		} else if (resolutionLC !== 'instant' || resolutionLC !== 'hourly' || resolutionLC !== 'daily') {
...
module.exports = {
	fetchUserProfile: () => simpleGET('/api/v1/users/profile', 'data'),
